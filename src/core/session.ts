...
import type { Message, SessionMeta, SessionType, SessionStatus, AgentName } from "./types.js";
...
export class SessionManager {
  ...
  load(sessionId: string): SessionData {
    ...
    const raw: SessionMeta & { type?: SessionType } = JSON.parse(
      fs.readFileSync(path.join(dir, "meta.json"), "utf-8")
    );

    // Backwards compat: sessions without a type field default to "debate"
    if (!raw.type) {
      raw.type = "debate";
    }
    ...
    const rawTranscript = fs.readFileSync(transcriptPath, "utf-8").trim();
    const messages: Message[] = rawTranscript
  ...
  listSessions(): SessionMeta[] {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        sessions.push(raw as SessionMeta);
      } catch {
  ...
  filterSessions(opts: { type?: SessionType; statuses?: SessionStatus[]; olderThan?: Date }): SessionMeta[] {
